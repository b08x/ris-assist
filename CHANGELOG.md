## [unreleased]

### 🚀 Features

- *(knowledge)* Implement ServiceNow-shaped KB article pipeline

### 🐛 Bug Fixes

- Package PERSONA-SPEC for installed plugins; require asking write path

### 💼 Other

- Merge tag 'v1.0.0' into development

## [1.0.0] - 2026-08-06

### 🚀 Features

- Implement persona spec and all five skills; sync docs/demos
- Allow document upload as answer during setup interview
- Allow document upload as answer during comms-tune build interview

### 🐛 Bug Fixes

- Resolve selector collision inflating persona-card confidence badges

### 📚 Documentation

- Drop out-of-context boundary segment from persona demo

### ⚙️ Miscellaneous Tasks

- Initialize RIS Triage plugin with Claude Desktop scaffold
- Add .codemap/ to .gitignore
- Complete initial project setup with maintainer info and formatting
